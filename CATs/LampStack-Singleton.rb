...
name "LAMP Stack Singleton"
rs_ca_ver 20131202
short_description "![logo](https://s3.amazonaws.com/rs-pft/cat-logos/lamp.png)

Launches a 3-tier LAMP stack. Only one instance of this CloudApp is allowed for each user!"
...
define launch_servers(@lb_server, @app_server, @db_server, @ssh_key, @sec_group, @sec_group_rule_http, @sec_group_rule_http8080, @sec_group_rule_mysql, @placement_group, $map_cloud, $map_st, $map_db_creds, $param_location, $inAzure, $invSphere, $needsSshKey, $needsPlacementGroup, $needsSecurityGroup)  return @lb_server, @app_server, @db_server, @sec_group, @ssh_key, $site_link, $lb_status_link do
  ...
  call sys_get_launched_from() retrieve $launched_from_templatename

  $launched_by_tag = "selfservice:launched_by="+$launched_by_user
  $launched_from_tag = "selfservice:launched_from="+$launched_from_templatename

  $by_tag_params = {
    match_all: "true",
    resource_type: "deployments",
    tags: [$launched_by_tag,$launched_from_tag]
  }
  ...
  if size($links) > 1
    raise "Only one instance of 'LAMP Stack Singleton' is allowed!"
  end
