...
define pre_auto_launch($map_cloud, $param_location, $map_st, $param_servertype, $map_mci, @windows_server) return @windows_server do
  ...
  $mci_name = $map_mci[$param_servertype][$param_location]
  $windows_server = to_object(@windows_server)
  @st = rs_cm.get(href: $windows_server["fields"]["server_template_href"])
  @mci = @st.multi_cloud_images(filter:["name=="+$mci_name])
  $windows_server["fields"]["multi_cloud_image_href"] = @mci.href
