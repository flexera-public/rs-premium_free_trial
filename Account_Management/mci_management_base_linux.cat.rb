...
parameter "param_base_arm_aus_image_name" do
  category "Inputs"
  label "Name root for the image name in AzureRM Australia."
  type "string"
  default "Canonical UbuntuServer 14.04.5-LTS latest"
end
...
define manage_mci($param_base_mci_name, $param_base_google_image_name, $param_pft_mci_name, $param_base_arm_aus_image_name) do
  ...
  $arm_aus_image_name_root = $param_base_arm_aus_image_name
  ...
  call update_google_image(@pft_base_mci, $google_image_name_root) retrieve $google_image_href

  call update_arm_aus_image(@pft_base_mci, $arm_aus_image_name_root) retrieve $arm_image_href

  # Ideally we would use a committed revision of this stuff
  # @pft_base_mci.commit(commit_message: "Updated GCE Ubuntu image to "+$google_image_href+".\r\nUpdated AzureRM Australia East Ubuntu image to "+$arm_image_href)
end
...
define update_google_image(@mci, $google_image_name_root) return $image_href do
  ...
  if (size(@cloud) > 0)
    ...
    call mci.mci_upsert_cloud_image(@mci, $cloud_href, $image_href)
  end
end

define update_arm_aus_image(@mci, $arm_aus_image_name_root) return $image_href do
  @cloud = rs_cm.clouds.get(filter: ["name==AzureRM Australia East"])

  if (size(@cloud) > 0)
    @images = @cloud.images(filter: ["name=="+$arm_aus_image_name_root])
    @image = last(@images)
    $cloud_href = @cloud.href
    $image_href = @image.href
